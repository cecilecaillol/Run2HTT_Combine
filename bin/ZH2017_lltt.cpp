...
int main(int argc, char **argv)
{
  ...
  //! [part3]
  cb.AddObservations({"*"}, {"zh"}, {"2017"}, {"lltt"}, cats);


  vector<string> bkg_procs = {"Triboson","Reducible","ZZ","ZH_hww125"};//VV
  cb.AddProcesses({"*"}, {"zh"}, {"2017"}, {"lltt"}, bkg_procs, cats, false);
  ...
  cb.AddProcesses(masses, {"zh"}, {"2017"}, {"lltt"}, sig_procs, cats, true);
  //! [part4]
  ...
  cb.cp().process({"Reducible"}).AddSyst(cb, "reducible_norm_lltt_stat_2017", "lnN", SystMap<>::init(1.02));
  ...
  cb.cp().process(JoinStr({{"Triboson","ZZ","ZH_hww125"},sig_procs})).AddSyst(cb, "CMS_eff_m_2017", "lnN", SystMap<>::init(1.02));
  ...
  TFile output((Input.ReturnToken(0)+"/"+"zh2017_lltt.input.root").c_str(), "RECREATE");
