...
int main(int argc, char **argv)
{
  ...
  //! [part3]
  cb.AddObservations({"*"}, {"zh"}, {"2018"}, {"llmt"}, cats);


  vector<string> bkg_procs = {"Triboson","Reducible","ZZ","ZH_hww125"};//VV
  cb.AddProcesses({"*"}, {"zh"}, {"2018"}, {"llmt"}, bkg_procs, cats, false);
  ...
  cb.AddProcesses(masses, {"zh"}, {"2018"}, {"llmt"}, sig_procs, cats, true);
  //! [part4]
  ...
  cb.cp().process({"Reducible"}).AddSyst(cb, "reducible_norm_llmt_stat_2018", "lnN", SystMap<>::init(1.03));
  ...
  cb.cp().process(JoinStr({{"Triboson","ZZ","ZH_hww125"},sig_procs})).AddSyst(cb, "CMS_eff_m_2018", "lnN", SystMap<>::init(1.036));
  ...
  TFile output((Input.ReturnToken(0)+"/"+"zh2018_llmt.input.root").c_str(), "RECREATE");
