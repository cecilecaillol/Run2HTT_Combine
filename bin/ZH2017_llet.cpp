...
int main(int argc, char **argv)
{
  ...
  //! [part3]
  cb.AddObservations({"*"}, {"zh"}, {"2017"}, {"llet"}, cats);


  vector<string> bkg_procs = {"Triboson","Reducible","ZZ","ZH_hww125"};//VV
  cb.AddProcesses({"*"}, {"zh"}, {"2017"}, {"llet"}, bkg_procs, cats, false);
  ...
  cb.AddProcesses(masses, {"zh"}, {"2017"}, {"llet"}, sig_procs, cats, true);
  //! [part4]
  ...
  cb.cp().process({"Reducible"}).AddSyst(cb, "reducible_norm_llet_stat_2017", "lnN", SystMap<>::init(1.03));
  cb.cp().process({"Reducible"}).AddSyst(cb,"CMS_fakeEle_promptSubtraction_2017", "lnN", SystMap<>::init(1.005));
  ...
  cb.cp().process(JoinStr({{"Triboson","ZZ","ZH_hww125"},sig_procs})).AddSyst(cb, "CMS_eff_e_2017", "lnN", SystMap<>::init(1.036));
  ...
  TFile output((Input.ReturnToken(0)+"/"+"zh2017_llet.input.root").c_str(), "RECREATE");
