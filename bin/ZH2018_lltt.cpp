...
int main(int argc, char **argv)
{
  ...
  //! [part3]
  cb.AddObservations({"*"}, {"zh"}, {"2018"}, {"lltt"}, cats);


  vector<string> bkg_procs = {"Triboson","Reducible","ZZ","ZH_hww125"};//VV
  cb.AddProcesses({"*"}, {"zh"}, {"2018"}, {"lltt"}, bkg_procs, cats, false);
  ...
  cb.AddProcesses(masses, {"zh"}, {"2018"}, {"lltt"}, sig_procs, cats, true);
  //! [part4]
  ...
  cb.cp().process({"Reducible"}).AddSyst(cb, "reducible_norm_lltt_stat_2018", "lnN", SystMap<>::init(1.02));
  ...
  TFile output((Input.ReturnToken(0)+"/"+"zh2018_lltt.input.root").c_str(), "RECREATE");
