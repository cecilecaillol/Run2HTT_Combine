...
int main(int argc, char **argv)
{
  ...
  // Create an llempty CombineHarvester instance that will hold all of the
  ...
  cb.AddObservations({"*"}, {"zh"}, {"2016"}, {"llem"}, cats);

  vector<string> bkg_procs = {"Triboson","Reducible","ZZ","ZH_hww125"};//VV
  cb.AddProcesses({"*"}, {"zh"}, {"2016"}, {"llem"}, bkg_procs, cats, false);
  ...
  cb.AddProcesses(masses, {"zh"}, {"2016"}, {"llem"}, sig_procs, cats, true);
  ...
  // Bkg normalization
  cb.cp().process({"Reducible"}).AddSyst(cb, "reducible_norm_llem_syst", "lnN", SystMap<>::init(1.15));
  cb.cp().process({"Reducible"}).AddSyst(cb, "reducible_norm_llem_stat_2016", "lnN", SystMap<>::init(1.03));
  cb.cp().process({"Reducible"}).AddSyst(cb,"CMS_fakeEle_promptSubtraction_2016", "lnN", SystMap<>::init(1.01));
  cb.cp().process({"Reducible"}).AddSyst(cb,"CMS_fakeMu_promptSubtraction_2016", "lnN", SystMap<>::init(1.02));
  ...
  cb.cp().process(JoinStr({{"Triboson","ZZ","ZH_hww125"},sig_procs})).AddSyst(cb, "CMS_eff_e_2016", "lnN", SystMap<>::init(1.032));
  cb.cp().process(JoinStr({{"Triboson","ZZ","ZH_hww125"},sig_procs})).AddSyst(cb, "CMS_eff_m_2016", "lnN", SystMap<>::init(1.032));
  ...
  // Observation, Process and Systllematic entries in the CombineHarvester
  // instance.

  // We create the output root file that will contain all the shapes.
  TFile output((Input.ReturnToken(0)+"/"+"zh2016_llem.input.root").c_str(), "RECREATE");
  ...
  for (auto b : bins) {
    for (auto m : masses) {
      ...
      // where we must rllemllember to include the "*" mass entry to get
