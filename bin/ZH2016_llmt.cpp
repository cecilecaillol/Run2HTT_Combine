...
int main(int argc, char **argv)
{
  ...
  //! [part3]
  cb.AddObservations({"*"}, {"zh"}, {"2016"}, {"llmt"}, cats);


  vector<string> bkg_procs = {"Triboson","Reducible","ZZ","ZH_hww125"};//VV
  cb.AddProcesses({"*"}, {"zh"}, {"2016"}, {"llmt"}, bkg_procs, cats, false);
  ...
  cb.AddProcesses(masses, {"zh"}, {"2016"}, {"llmt"}, sig_procs, cats, true);
  //! [part4]
  ...
  cb.cp().process({"Reducible"}).AddSyst(cb, "reducible_norm_llmt_stat_2016", "lnN", SystMap<>::init(1.03));
  cb.cp().process({"Reducible"}).AddSyst(cb,"CMS_fakeMu_promptSubtraction_2016", "lnN", SystMap<>::init(1.01));
  ...
  cb.cp().process(JoinStr({{"Triboson","ZZ","ZH_hww125"},sig_procs})).AddSyst(cb, "CMS_eff_m_2016", "lnN", SystMap<>::init(1.036));
  ...
  TFile output((Input.ReturnToken(0)+"/"+"zh2016_llmt.input.root").c_str(), "RECREATE");
